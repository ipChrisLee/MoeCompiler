...
antlrcpp::Any ASTVisitor::visitAdd2(SysYParser::Add2Context * ctx) {
	...
	if (info.stat.calculatingStaticValue) {
		...
		ctx->addExp()->accept(this);
		auto resL = retVal.restore<ircode::AddrOperand *>();
		auto instrsL = retInstrs.restore<std::list<ircode::IRInstr *>>();
		instrsRes.splice(instrsRes.end(), std::move(instrsL));
		ctx->mulExp()->accept(this);
		auto resR = retVal.restore<ircode::AddrOperand *>();
...
antlrcpp::Any ASTVisitor::visitUnary2(SysYParser::Unary2Context * ctx) {
	...
	if (info.stat.calculatingStaticValue) {
		...
		auto pAddrFunc = dynamic_cast<ircode::AddrFunction *>(_addrFunc);
		...
		retVal.save(static_cast<ircode::AddrOperand *>(pRetValAddr));
		retInstrs.save(std::move(instrsRes));
		return nullptr;
	}
}
...
antlrcpp::Any ASTVisitor::visitLVal(SysYParser::LValContext * ctx) {
	...
	auto [identifierType, pIDVarAddr] = [
		this, &varName
	]() -> std::pair<IdType, ircode::AddrVariable *> {
		...
		auto * _pIDVarAddr = dynamic_cast<ircode::AddrVariable *>(pIDAddr);
	...
	if (info.stat.calculatingStaticValue) {
		...
		switch (identifierType) {
			...
			case IdType::LocalVarName: {
				auto * pAddr = dynamic_cast<ircode::AddrLocalVariable *>(pIDVarAddr);
				com::Assert(
	...
	if (com::enum_fun::in(idPointToTypeInfo.type, {Type::Float_t, Type::Int_t})) {
		if (info.stat.visitingAssignmentLeft) { // as left side of assignment
			...
			auto * pValAddr = ir.addrPool.emplace_back(
				ircode::AddrVariable(idPointToTypeInfo)
			);
			instrsRes.emplace_back(
				ir.instrPool.emplace_back(
					ircode::InstrLoad(pIDVarAddr, pValAddr)
				)
			);
			retVal.save(static_cast<ircode::AddrOperand *>(pValAddr));
			retInstrs.save(std::move(instrsRes));
			return nullptr;
		}
	}
	...
	auto * pDerefOnceAddr = [
		...
		auto * pDerefOnceAddr = ir.addrPool.emplace_back(
			ircode::AddrVariable(*typeDeduce(idTypeInfo, 1))
		);
		if (idType == IdType::ParameterName) {
			instrsRes.emplace_back(
				ir.instrPool.emplace_back(
					ircode::InstrLoad(pIDVarAddr, pDerefOnceAddr)
				)
			);
		} else {
			auto idxs = std::vector<ircode::AddrOperand *>(
	...
	if (info.stat.visitingAssignmentLeft) { // as left side of assignment
		...
		//  addr for val
		auto pValAddr = ir.addrPool.emplace_back(
			ircode::AddrVariable(*typeDeduce(pDerefOnceAddr->getType(), nExp))
		);
		switch (pValAddr->getType().type) {
			...
			case Type::Int_t: {
				auto pValMemAddr = ir.addrPool.emplace_back(
					ircode::AddrVariable(PointerType(IntType()))
				);
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrGetelementptr(
				...
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrLoad(pValMemAddr, pValAddr)
					)
				);
				break;
			}
			case Type::Float_t: {
				auto pValMemAddr = ir.addrPool.emplace_back(
					ircode::AddrVariable(PointerType(FloatType()))
				);
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrGetelementptr(
...
antlrcpp::Any ASTVisitor::visitNumber(SysYParser::NumberContext * ctx) {
	...
	if (info.stat.calculatingStaticValue) {
		...
		auto pAddr = static_cast<ircode::AddrOperand *>(nullptr);
		if (ctx->FloatLiteral()) {
			pAddr = ir.addrPool.emplace_back(
				ircode::AddrStaticValue(FloatStaticValue(ctx->getText()))
			);
		} else if (ctx->IntLiteral()) {
			pAddr = ir.addrPool.emplace_back(
				ircode::AddrStaticValue(IntStaticValue(ctx->getText()))
			);
		} else {
			com::Throw("ctx should be float/int literal.", CODEPOS);
		}
		retVal.save(std::move(pAddr));//NOLINT
		retInstrs.save(std::list<ircode::IRInstr *>());
		return nullptr;
	}
}
...
antlrcpp::Any ASTVisitor::visitLOr2(SysYParser::LOr2Context * ctx) {
	...
	instrsRes.emplace_back(
		ir.instrPool.emplace_back(ircode::InstrLabel(pLabelLeftFalse))//NOLINT
	);
...
antlrcpp::Any ASTVisitor::visitLAnd1(SysYParser::LAnd1Context * ctx) {
	...
	auto * pOpAddr = retVal.restore<ircode::AddrOperand *>();
	auto instrsOperand = retInstrs.restore<std::list<ircode::IRInstr *>>();
	//  Convert to bool
...
antlrcpp::Any ASTVisitor::visitLAnd2(SysYParser::LAnd2Context * ctx) {
	...
	instrsRes.emplace_back(
		ir.instrPool.emplace_back(ircode::InstrLabel(pLabelLeftTrue))//NOLINT
	);
	ctx->eqExp()->accept(this);
	auto * pExpResAddr = retVal.restore<ircode::AddrOperand *>();
	instrsRes.splice(
		instrsRes.end(), retInstrs.restore<std::list<ircode::IRInstr *>>()
	);
...
antlrcpp::Any ASTVisitor::visitEq2(SysYParser::Eq2Context * ctx) {
	...
	ctx->eqExp()->accept(this);
	auto * pLeftResAddr = retVal.restore<ircode::AddrOperand *>();
	auto instrsLeft = retInstrs.restore<std::list<ircode::IRInstr *>>();
	ctx->relExp()->accept(this);
	auto * pRightResAddr = retVal.restore<ircode::AddrOperand *>();
	auto instrsRight = retInstrs.restore<std::list<ircode::IRInstr *>>();
	//  Conversion
	...
	retVal.save(static_cast<ircode::AddrOperand *>(pResAddr));
	retInstrs.save(std::move(instrsRes));
	return nullptr;
}
