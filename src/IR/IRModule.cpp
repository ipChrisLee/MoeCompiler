...
std::list<ircode::IRInstr *> genCondInstrs(
	...
	switch (opR->getType().type) {
		case Type::Int_t: {
			...
			if (testNEQ0) {
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrICmp(
							opD, opR, ircode::ICMP::NE, pZeroAddr
						)
					)
				);
			} else {
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrICmp(
							opD, opR, ircode::ICMP::EQ, pZeroAddr
						)
					)
				);
			}
		...
		case Type::Float_t: {
			...
			if (testNEQ0) {
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrFCmp(
							opD, opR, ircode::FCMP::UNE, pZeroAddr
						)
					)
				);
			} else {
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
						ircode::InstrFCmp(
							opD, opR, ircode::FCMP::OEQ, pZeroAddr
						)
					)
				);
			}
		...
		case Type::Bool_t: {
			auto * pZeroAddr = ir.addrPool.emplace_back(
				ircode::AddrStaticValue(BoolStaticValue(false))
			);
			if (testNEQ0) {
				instrsRes.emplace_back(
					ir.instrPool.emplace_back(
		...
		default: com::Throw("LVal in condition should be int/float/bool!", CODEPOS);
	}
	return instrsRes;
}
