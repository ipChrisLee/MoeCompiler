...
public:
    ...
    void infoPrint(
        ...
        if(enable && sysEnable){
            if(decorated){
                if(printLoc.length()) printer->cprintLn(headstr+printLoc);
                printer->cprintLn(headstr+std::string(t.first)+" : "+cprt::tos(t.second));
            }else{
                if(printLoc.length()) printer->printLn(headstr+printLoc);
    ...
    void infoPrint(\
        ...
        if(decorated){
            if(printLoc.length()) printer->cprintLn(headstr+printLoc);
            apply([&](auto && ... args){ 
                ((printer->cprintLn(headstr+std::string(args.first)+" : "+cprt::tos(args.second))),...);
            },t);
        }else{
            if(printLoc.length()) printer->printLn(headstr+printLoc);
            apply([&](auto && ... args){ 
                ((printer->printLn(headstr+std::string(args.first)+" : "+cprt::tos(args.second))),...);
            },t);
        }
    }
};
