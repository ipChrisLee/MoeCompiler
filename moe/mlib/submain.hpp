...
namespace com{
    ...
    using mainFun_t=std::function<int(std::vector<std::string>)>;
    ...
    class submain{
      public:
        struct SubMainCase{
            ...
            SubMainCase(){}
            SubMainCase(mainFun_t mainFunction):positions(),mainFunction(mainFunction){}
            SubMainCase & operator = (const SubMainCase &)=default;
        };
      private:
        static int errCase(std::vector<std::string>){
            return 0;
        }
      ...
      public:
        ...
        const std::map<std::string,SubMainCase> & getCases() const { return cases; }
