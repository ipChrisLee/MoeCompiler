...
/*bool Live_at(ud_chain *c1,ud_chain *c2,size_t pos){
    std::set<size_t> active;
   for(size_t use:c1->use_in){
        if(use>pos) active.insert(use);
   }
   for(size_t use2:c2->use_in){
        if(active.count(use2)) return true;
   }
   return false;
}*/
